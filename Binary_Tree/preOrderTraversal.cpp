...
void preOrderTraversal(TreeNode<int> *root, vector<int> &rootData){
    rootData.push_back(root->data);
    if(root->left){
        preOrderTraversal(root->left, rootData);
    }
    if(root->right){
        preOrderTraversal(root->right, rootData);
    }
}
vector<int> preOrder(TreeNode<int> * root){
    // Write your code here.
    vector<int> rootData;
    preOrderTraversal(root, rootData);
